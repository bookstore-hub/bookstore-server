...
public interface BookRepository extends CrudRepository<Book, Long> {
    ...
    @Query(value = "SELECT DISTINCT * FROM bookstore.Book b WHERE SIMILARITY(b.title, ?1) > 0.1 ORDER BY similarity(b.title, ?1) DESC", nativeQuery = true)
    List<Book> findByTitle(String title);

    @Query(value = "SELECT DISTINCT b.* FROM bookstore.book b " +
